...
public class FIFOClosedTransactionModel
{
    ...
    public static final String SELECT_ALL_BY_JOOMLAID_EQUITYTYPE
        = "select DMAcctId, JoomlaId, FiTId, TransactionGrp, Ticker, EquityId, TransactionName, DateOpen, DateClose, DateExpire, Units, PriceOpen, PriceClose, TotalOpen, TotalClose, Gain, GainPct, EquityType, PositionType, Transactiontype, Complete, Days from hlhtxc5_dmOfx_dev.FIFOClosedTransactions where JoomlaId = '%s' and EquityType = '%s' order by EquityId, DateOpen, DateClose ;";

    //limit to current year
    public static final String SELECT_INCOMPLETE_BY_JOOMLAID_EQUITYTYPE
        = "select DMAcctId, JoomlaId, FiTId, TransactionGrp, Ticker, EquityId, TransactionName, DateOpen, DateClose, DateExpire, Units, PriceOpen, PriceClose, TotalOpen, TotalClose, Gain, GainPct, EquityType, PositionType, Transactiontype, Complete, Days from hlhtxc5_dmOfx_dev.FIFOClosedTransactions where Complete <> 1 and JoomlaId = '%s' and EquityType = '%s' order by EquityId, DateOpen, DateClose";

    public static final String INSERT_CLOSED_OPTION_FIFO_2_FIFO_CLOSED_TRANSACTIONS
        = "insert ignore into hlhtxc5_dmOfx_dev.FIFOClosedTransactions (DMAcctId, JoomlaId, FiTId, TransactionGrp, EquityId, TransactionName, Ticker, DateOpen, DateClose, Units, PriceOpen, PriceClose, DateExpire, Days, PositionType, TotalOpen, TotalClose, EquityType, Gain, GainPct, TransactionType) select ClosedOptionFIFO.DMAcctId, ClosedOptionFIFO.JoomlaId, FiTId, TransactionGrp, EquityId, TransactionName, ClosedOptionFIFO.Ticker, DateOpen, DateClose, Units, PriceOpen, PriceClose, DateExpire, datediff(DateExpire, CurDate()) as `Days`, if(TotalOpen < 0, 'LONG', 'SHORT') as PositionType, TotalOpen, TotalClose, 'OPTION' as EquityType, (TotalOpen + TotalClose) as Gain, if (TotalOpen = 0, 0, round(100 * (TotalClose + TotalOpen) / abs(TotalOpen), 4)) as GainPct, TransactionType from hlhtxc5_dmOfx_dev.ClosedOptionFIFO where ClosedOptionFIFO.JoomlaId = '%s'";

    public static final String INSERT_CLOSED_STOCK_FIFO_2_FIFO_CLOSED_TRANSACTIONS
        = "insert ignore into hlhtxc5_dmOfx_dev.FIFOClosedTransactions (DMAcctId, JoomlaId, FiTId, TransactionGrp, EquityId, Ticker, DateOpen, DateClose, Units, PriceOpen, PriceClose, PositionType, TotalOpen, TotalClose, EquityType, Gain, GainPct, TransactionType, Days) select csf.DMAcctId, csf.JoomlaId, FiTId, TransactionGrp, EquityId, csf.Ticker, DateOpen as 'Open', DateClose as `Close`, Units, PriceOpen, PriceClose, if(csf.TransactionType = 'BUY', 'LONG', if (csf.TransactionType = 'SELLSHORT', 'SHORT', 'UNKNOWN')) as PositionType, TotalOpen, TotalClose, 'STOCK' as EquityType, (TotalClose + TotalOpen) as Gain, round(100 * (TotalClose + TotalOpen) / abs(TotalOpen), 4) as GainPct, csf.TransactionType, 0 as Days from hlhtxc5_dmOfx_dev.ClosedStockFIFO as csf where csf.JoomlaId = '%s'";

    public static final String UPDATE_COMPLETE
        = "update hlhtxc5_dmOfx_dev.FIFOClosedTransactions set Complete = '%s' where DMAcctId = '%s' and JoomlaId = '%s' and FiTId = '%s'";
