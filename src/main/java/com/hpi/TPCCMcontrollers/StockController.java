...
public class StockController
{
    ...
    public void getDistinctEquityId(Integer account)
    {
        ...
        sql
            = "select distinct A.EquityId from (select EquityId from hlhtxc5_dmOfx_dev.OpeningStock where DMAcctId = '%s' and JoomlaId = '%s' and not(Units = 0) union select EquityId from hlhtxc5_dmOfx_dev.ClientOpeningStock where DMAcctId = '%s' and JoomlaId = '%s' and not(Units = 0) union select EquityId from hlhtxc5_dmOfx_dev.ClosingStock where DMAcctId = '%s' and JoomlaId = '%s' and not(Units = 0) union select EquityId from hlhtxc5_dmOfx_dev.ClientClosingStock where DMAcctId = '%s' and JoomlaId = '%s' and not(Units = 0)) as A order by EquityId;";
...
////        sql =
////            "select * from hlhtxc5_dmOfx_dev.OpeningStock where DMAcctId = '%s' and EquityId = '%s' and JoomlaId = '%s' and Units <> 0 union select %s from hlhtxc5_dmOfx_dev.clientOpeningStock where DMAcctId = '%s' and EquityId = '%s' and JoomlaId = '%s' and Units <> 0 order by EquityId, GMTDtTrade, FiTId;";
//
//        sql = String.format(sql,
...
//        sql =
//            "select * from hlhtxc5_dmOfx_dev.ClosingStock where DMAcctId = '%s' and EquityId = '%s' and JoomlaId = '%s' and not(Units = 0) union select %s from hlhtxc5_dmOfx_dev.clientClosingStock where DMAcctId = '%s' and EquityId = '%s' and JoomlaId = '%s' and not(Units = 0) order by EquityId, GMTDtTrade, FiTId;";
//        sql = String.format(sql,
//            account, equityId, this.userId,
...
//        closingStock:
        for (ClosingStockModel cs : this.stockClosingList)
        {
            ...
            sql
                = "update hlhtxc5_dmOfx_dev.ClosingStock set Units = '%s' where DMAcctId = '%s' and JoomlaId = '%s' and FiTId = '%s';";
        ...
        sql
            = "update hlhtxc5_dmOfx_dev.OpeningStock set Units = '%s' where DMAcctId = '%s' and JoomlaId = '%s' and FiTId = '%s';";
        CMDBController.executeSQL(String.format(sql,
            os.getUnits(),
            os.getDmAcctId(),
            os.getJoomlaId(),
